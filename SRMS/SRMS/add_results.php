...
<?php
    if(isset($_POST['rno'],$_POST['p1'],$_POST['p2'],$_POST['p3'],$_POST['p4'],$_POST['p5'],$_POST['p6']))
    {
        ...
        // validation
        if (empty($class_name) or empty($rno) or empty($dept) or empty($exam) or $p1>100 or  $p2>100 or $p3>100 or $p4>100 or $p5>100 or $p1<0 or  $p2<0 or $p3<0 or $p4<0 or $p5<0 ) {
            if(empty($class_name))
                echo '<p class="error">Please select class</p>';
            if(empty($dept))
                echo '<p class="error">Please select Department</p>';
            if(empty($exam))
                echo '<p class="error">Please select Exam pattern</p>';
            if(empty($class_name))
                echo '<p class="error">Please select class</p>';
            if(empty($rno))
        ...
        $sql="INSERT INTO `result` (`name`, `rno`, `class`, `dept`, `exam`, `sub1`, `p1`, `sub2`, `p2`, `sub3`, `p3`, `sub4`, `p4`, `sub5`, `p5`, `sub6`, `p6`, `marks`, `percentage`) VALUES ('$display', '$rno', '$class_name', '$dept', '$exam', '$sub1', '$p1', '$sub2', '$p2', '$sub3', '$p3', '$sub4', '$p4', '$sub5', '$p5', '$sub6', '$p6', '$marks', '$percentage')";
